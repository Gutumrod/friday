...
local Thai TTS candidate ที่ฟังดูใช้ได้จริง หลัง 2 ตัวก่อนหน้าพังทั้งคู่ **แต่ยังเป็นแค่ผล demo
ยังไม่ได้ integrate เข้า Friday จริง** งาน integration (`f5-tts` glue code, ตัดสินใจ CPU/GPU,
ระวังไม่ให้ซ้ำรอย WSL crash) ยังไม่ได้เริ่ม เก็บไว้เป็นงานของ session ถัดไปที่มีเวลา/สมองพร้อมกว่านี้

---

## ถัดไป (สำหรับ session ใหม่)

1. **รอ CEO ทดสอบปากจริง** (mic threshold + Google Cloud STT) — ถ้าโอเค commit ทันที ถ้าไม่โอเค
   ไล่แยกว่าจุดไหนเป็นสาเหตุ (อาจต้องเปิด verbose logging เพิ่มถ้ายังไม่ชัด)
...
4. **JaiTTS integration** — ถ้า CEO อยากทำต่อ: เขียน glue code ต่อ checkpoint (`model.pt`+`vocab.txt`)
   เข้ากับ `f5-tts` package จริง (ไม่ใช่ `flowtts` ที่ไม่มีจริง), ตัดสินใจ CPU vs GPU (ระวังไม่ให้ซ้ำ
   WSL2/Fish Audio CUDA OOM เดิม), ทดสอบ code-switching จริงกับประโยคที่ Friday เจอจริง (ชื่อแอป/
   คำสั่ง TV) ไม่ใช่แค่ demo ทั่วไป
...
Commander (Claude) — session 2026-07-03 เย็น ถึง 2026-07-04 ดึก
