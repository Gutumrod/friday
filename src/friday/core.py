...
def load_facts():
    return _memory.load_facts(FACTS_PATH)

SESSION_FILE_PATTERN = _memory.SESSION_FILE_PATTERN
LEGACY_DAY_FILE_PATTERN = _memory.LEGACY_DAY_FILE_PATTERN

_current_session_path = None  # set by start_new_session(); log_to_vault() writes here

def migrate_legacy_day_files():
    return _memory.migrate_legacy_day_files(HISTORY_DIR)

def start_new_session():
    global _current_session_path
    session_number, path = _memory.start_new_session(HISTORY_DIR)
...
def log_to_vault(role, text):
    ...
    if _current_session_path is None:
        start_new_session()
    _memory.log_to_vault(_current_session_path, role, text)
...
def ask_ollama(prompt, history, tools=None):
    ...
    return _llm.ask_ollama(
        ...
        speak_fn=speak,
        slow_warning_message=SLOW_WARNING_MESSAGE,
...
# FIRE_REMINDER_SCRIPT comes from friday.config so core.py can live under src/friday/.
